#!/usr/bin/env python
# -*- coding: utf-8; explicit-buffer-name: "scaleCell.py<nrf2lib>" -*-
    ...
    for net in sourceCell.getNets():
      ...
      for component in net.getComponents():
        ...
        if isinstance(component,Contact):
          ...
          if sourceCell.getName() == 'rf2_mid_mem' or sourceCell.getName() == 'rf2_mid_mem_r0':
            if component.getLayer().getName() == 'CONT_BODY_N':
              if     component.getX() == DbU.fromLambda(29.0) \
                 and component.getY() == DbU.fromLambda(39.0):
                print '[INFO] Shifted down CONT_BODY_N contact (rf2_mid_mem).'
                print '       %s' % component
                custom_dY = DbU.fromLambda(-1.0)

          if sourceCell.getName() == 'rf2_inmux_buf':
            if component.getLayer().getName() == 'CONT_BODY_N':
              if     component.getX() == DbU.fromLambda(21.0) \
                 and component.getY() == DbU.fromLambda(54.0):
                print '[INFO] Shifted right CONT_BODY_N contact (rf2_inmux_buf).'
                print '       %s' % component
                custom_dX = DbU.fromLambda(2.0)

          if sourceCell.getName() == 'rf2_mid_buf':
            if component.getLayer().getName() == 'CONT_BODY_N':
              if     component.getX() == DbU.fromLambda( 0.0) \
                 and component.getY() == DbU.fromLambda(28.0):
                print '[INFO] Deleted CONT_BODY_N contact (rf2_inmux_buf).'
                print '       %s' % component
                continue
              if     component.getX() == DbU.fromLambda(12.0) \
                 and component.getY() == DbU.fromLambda(28.0):
                print '[INFO] Deleted CONT_BODY_N contact (rf2_inmux_buf).'
                print '       %s' % component
                continue
              if     component.getX() == DbU.fromLambda(35.0) \
                 and component.getY() == DbU.fromLambda(28.0):
                print '[INFO] Deleted CONT_BODY_N contact (rf2_inmux_buf).'
                print '       %s' % component
                continue

          if sourceCell.getName() == 'rf2_out_mem':
            if component.getLayer().getName() == 'CONT_DIF_P':
              if     component.getX() == DbU.fromLambda(27.0) \
                 and component.getY() == DbU.fromLambda(45.0):
                print '[INFO] Shifted down CONT_DIF_P contact (to reconnect).'
                print '       %s' % component
                custom_dY = DbU.fromLambda(-1.0)
              if     component.getX() == DbU.fromLambda(78.0) \
                 and component.getY() == DbU.fromLambda(45.0):
                print '[INFO] Shifted down CONT_DIF_P contact (to reconnect).'
                print '       %s' % component
                custom_dY = DbU.fromLambda(-1.0)
         ...
         # are already scaled (to the minimal size).
          ...
          if sourceCell.getName() == 'rf2_out_mem':
            if component.getLayer().getName() == 'NDIF':
              if     component.getSourceY() == DbU.fromLambda(  4.0) \
                 and component.getTargetY() == DbU.fromLambda( 17.0) \
                 and component.getX()       == DbU.fromLambda( 53.0) \
                 and component.getWidth()   == DbU.fromLambda(  3.0):
                print '[INFO] Enlarge NDIF segment %s.' % component
            ...
            if component.getLayer().getName() == 'PDIF':
              if     component.getSourceY() == DbU.fromLambda( 28.0) \
                 ...
                 and component.getX()       == DbU.fromLambda( 53.0) \
                 and component.getWidth()   == DbU.fromLambda(  3.0):
                print '[INFO] Enlarge PDIF segment %s.' % component
                dW += DbU.fromLambda(2.0)
