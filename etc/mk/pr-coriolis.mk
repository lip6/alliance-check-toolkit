...
%_r.gds: $(CORE_NETLIST).blif
	-$(call scl_cols,eval `$(CORIOLIS_TOP)/etc/coriolis2/coriolisEnv.py $(DEBUG_OPTION)`; $(DoCHIP) $(DoCHIP_FLAGS) -prS --lef=$(LEF_LIBS) --blif=$*)

cgt-run: $(CORE_NETLIST).blif
	$(call scl_cols,eval `$(CORIOLIS_TOP)/etc/coriolis2/coriolisEnv.py $(DEBUG_OPTION)`; $(VALGRIND_COMMAND) cgt -V --script=loadDesign)
...
%_cts_r.ap  %_cts_r.vst  %_cts.vst:  $(CHIP).vst ./coriolis2/ioring.py $(CORONA).vst $(NETLISTS_PNR_VST)
	...
	-$(call scl_cols,eval `$(CORIOLIS_TOP)/etc/coriolis2/coriolisEnv.py $(DEBUG_OPTION)`; $(DoCHIP) $(DoCHIP_FLAGS) -prCTS --cell=$(CHIP))
	-touch *_cts.*
...
%_cts_r.ap  %_cts_r.vst  %_cts.vst:  ./coriolis2/ioring.py $(NETLISTS_PNR_VST)
	...
	-$(call run_if_older,$@,$(CORE_NETLIST),$(call scl_cols,eval `$(CORIOLIS_TOP)/etc/coriolis2/coriolisEnv.py $(DEBUG_OPTION)`; $(DoCHIP) $(DoCHIP_FLAGS) -prGCTS --cell=$(CORE_NETLIST)))
	-touch *_cts.*
...
%_cts_r.ap  %_cts_r.vst  %_cts.vst:  ./coriolis2/ioring.py $(NETLISTS_PNR_VST) $(DESIGN).py
	...
	-$(call scl_cols,eval `$(CORIOLIS_TOP)/etc/coriolis2/coriolisEnv.py $(DEBUG_OPTION)`; $(DoCHIP) $(DoCHIP_FLAGS) -prCTS --script=$(DESIGN))
	-touch *_cts.*

%_cts_r.ap  %_cts_r.vst  %_cts.vst:  ./coriolis2/ioring.py  $(NETLISTS_PNR_VST)
	-@echo "Using implicit rule for CORE/BLOCK creation from a VST netlist (clock tree enabled)."
	-$(call scl_cols,eval `$(CORIOLIS_TOP)/etc/coriolis2/coriolisEnv.py $(DEBUG_OPTION)`; $(DoCHIP) $(DoCHIP_FLAGS) -prCTS --cell=$(CORE_NETLIST))
...
%_cts_r.ap  %_cts_r.vst  %_cts.vst:  $(NETLISTS_PNR_VST)
	...
	-$(call scl_cols,eval `$(CORIOLIS_TOP)/etc/coriolis2/coriolisEnv.py $(DEBUG_OPTION)`; $(DoCHIP) $(DoCHIP_FLAGS) -prTS --cell=$*)
...
%_r.ap  %_r.vst:  $(NETLISTS_PNR_VST) $(DESIGN).py ./coriolis2/ioring.py
	...
	-$(call scl_cols,eval `$(CORIOLIS_TOP)/etc/coriolis2/coriolisEnv.py $(DEBUG_OPTION)`; $(DoCHIP) $(DoCHIP_FLAGS) --script=$(DESIGN))

%_r.ap  %_r.vst:  $(NETLISTS_PNR_VST) ./coriolis2/ioring.py 
	-@echo "Using implicit rule for manually created CHIP/CORONA (no clock tree)."
	-$(call scl_cols,eval `$(CORIOLIS_TOP)/etc/coriolis2/coriolisEnv.py $(DEBUG_OPTION)`; $(DoCHIP) $(DoCHIP_FLAGS) -prCS --cell=$*)

%_r.ap  %_r.vst: $(NETLISTS_PNR_VST) %.ap
	-@echo "Using implicit rule for routing an already placed design."
	-$(call scl_cols,eval `$(CORIOLIS_TOP)/etc/coriolis2/coriolisEnv.py $(DEBUG_OPTION)`; $(DoCHIP) $(DoCHIP_FLAGS) -rS --cell=$*)

%_r.ap  %_r.vst: $(NETLISTS_PNR_VST)
	-@echo "Using implicit rule for CORE/BLOCK creation from a VST netlist (no clock tree)."
	-$(call scl_cols,eval `$(CORIOLIS_TOP)/etc/coriolis2/coriolisEnv.py $(DEBUG_OPTION)`; $(DoCHIP) $(DoCHIP_FLAGS) -prS --cell=$*)
