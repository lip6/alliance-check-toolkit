...
def setupSg13g2_vsc ( checkToolkit=None ):
    Where( checkToolkit )
    ShellEnv().export()

    pdkDir          = Where.checkToolkit / 'dks' / 'sg13g2_vsc' / 'libs.tech'
    coriolisTechDir = pdkDir / 'coriolis'
    if not pdkDir.is_dir():
        print( '[ERROR] technos.setupSg13g2_vsc(): PDK directory do *not* exists:' )
        print( '        "{}"'.format(techDir.as_posix()) )
    sys.path.append( coriolisTechDir.as_posix() )

    cellsTop  = Where.checkToolkit / 'cells'
    liberty   = coriolisTechDir / 'sg13g2_vsc' / 'vsclib.lib'

    from coriolis          import Cfg 
    from coriolis          import Viewer
    from coriolis          import CRL 
    from coriolis.helpers   import overlay, l, u, n
    from sg13g2_vsc import techno, vsclib
