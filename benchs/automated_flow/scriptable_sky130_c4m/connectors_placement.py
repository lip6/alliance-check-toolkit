...
#Horizontal and veritcal sizes are in µm but pitchs are expressed in units
def connectors_number_per_side(horizontal_size,vertical_size,horizontal_pitch,vertical_pitch,unit,N):
    # we take a marge of one pitch at each corner
    north_side_capacity = horizontal_size -(2*horizontal_pitch*unit)
    south_side_capacity = horizontal_size -(2*horizontal_pitch*unit)
    east_side_capacity  = vertical_size   -(2*vertical_pitch*unit)
    west_side_capacity  = vertical_size   -(2*vertical_pitch*unit)
    ...
    north_spacing = int(north_side_capacity /(north_side_pins_number * horizontal_pitch*unit))
    south_spacing = int(south_side_capacity /(south_side_pins_number * horizontal_pitch*unit))
    east_spacing  = int(east_side_capacity  /(east_side_pins_number  * vertical_pitch*unit)  )
    west_spacing  = int(west_side_capacity  /(west_side_pins_number  * vertical_pitch*unit)  )
...
def signals_per_side(dico,horizontal_size,vertical_size,horizontal_pitch,vertical_pitch,unit):
  N = number_of_bits(dico)
  sorted_indexes_dico =   order_of_placement(dico)
  sides_connectors_number = connectors_number_per_side(horizontal_size,vertical_size,horizontal_pitch,vertical_pitch,unit,N)[0]
  sides_connectors_pitches = connectors_number_per_side(horizontal_size,vertical_size,horizontal_pitch,vertical_pitch,unit,N)[1]
  north=[]
  south=[]
  east =[]
  west =[]
...
def generate_ioPinsSpec(dico,horizontal_size,vertical_size,horizontal_pitch,vertical_pitch,unit):
    spacings,connectors =  signals_per_side(dico,horizontal_size*unit,vertical_size*unit,horizontal_pitch,vertical_pitch,unit)
...
#def generate_ioPinsSpec(dico,horizontal_size,vertical_size,horizontal_pitch,vertical_pitch,unit):
#    spacings,connectors =  signals_per_side(dico,horizontal_size,vertical_size,horizontal_pitch,vertical_pitch,unit)
#    ioPinsSpec = []
#    side=['NORTH','SOUTH','EAST','WEST']
#    pitch_id=['m2pitch','m2pitch','m1pitch','m1pitch']
#    for s in range (4):
...
#     side_name =   f'IoPin.{side[s]} |IoPin.A_BEGIN' 
#     for i in range (len(connectors[s])):
#        if  connectors[s][i][2] != 0: ##vector not bit 
#            spacing_between_bits = f'{spacings[s]}*{pitch_id[s]}'
#        else:
#            spacing_between_bits = 0
#        if connectors[s][i][3] == 'bit':
...
def generate_doDesign_file(entity,all_signals,horizontal_area,vertical_area,horizontal_pitch,vertical_pitch,unit,comb,clk_name='clk',reset_name='reset'):
 test = generate_ioPinsSpec(all_signals,horizontal_area,vertical_area,horizontal_pitch,vertical_pitch,unit)
...
def routine(file,entity,h_area,v_area,horizontal_pitch,vertical_pitch,unit,comb,clk_name='clk',reset_name='reset'):

 generate_dodo_file(entity,comb)
 cmd = 'cp init_template_doDesign.py doDesign.py'
 os.system (cmd)
 all_signals=synthesis_and_initialize(entity)
 generate_doDesign_file(entity,all_signals,h_area,v_area,horizontal_pitch,vertical_pitch,unit,comb,clk_name,reset_name)
 cmd_pnr=  f'../../../bin/crlenv.py doit pnr >debug_{entity}.txt'
 os.system(cmd_pnr)

def routine_with_sta(file,entity,h_area,v_area,horizontal_pitch,vertical_pitch,unit,comb,lib,model,clk_name='clk',reset_name='reset'):
 routine(file,entity,h_area,v_area,horizontal_pitch,vertical_pitch,unit,comb,clk_name='clk',reset_name='reset')
 if comb ==  1:
...
##print(dico,h,v)
#signals_per_side(dico,h,v,horizontal_pitch,vertical_pitch,unit)
