...
for i in cell.getExternalNets():
    ...
    if vector in vectors:
     ...
     vectors[vector][2]=min(vectors[vector][2],a.split(')')[0].split('(')[1])
    else:
        supply = False
...
def scriptMain ( **kw ):
    ...
    try:
       ...
       #Breakpoint.setStopLevel( 100 )
        ...
        if buildChip:
            cellName += '_harness'
        cell = af.getCell( 'dummy', CRL.Catalog.State.Logical )
        if editor:
            editor.setCell( cell ) 
