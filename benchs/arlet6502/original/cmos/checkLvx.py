...
def scriptMain ( **kw ):
    ...
    with overlay.CfgCache(priority=Cfg.Parameter.Priority.UserFile) as cfg:
        ...
        cfg.misc.minTraceLevel          = 160
        cfg.misc.maxTraceLevel          = 169
   ...
   #cell = af.getCell( 'inv_x1', CRL.Catalog.State.Views )
    cell = af.getCell( 'sff2_x4', CRL.Catalog.State.Views )
   #cell = af.getCell( 'tie_x0', CRL.Catalog.State.Views )
   #cell = af.getCell( 'buf_x8', CRL.Catalog.State.Views )
   #cell = af.getCell( 'rowend_x0', CRL.Catalog.State.Views )
    if editor:
        editor.setCell( cell ) 
        editor.setDbuMode( DbU.StringModePhysical )
    print( 'Running tramontana' )
    extractor = TramontanaEngine.create( cell )
    extractor.extract()
