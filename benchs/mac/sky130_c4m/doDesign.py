...
def scriptMain ( **kw ):
    ...
    try:
       ...
       #Breakpoint.setStopLevel( 99 )
        ...
        if buildChip:
            cellName += '_harness'
        cell = af.getCell( 'mac', CRL.Catalog.State.Logical )
        if editor:
            editor.setCell( cell ) 
        ...
        if buildChip:
            ...
            m2pitch    = u(0.51)
            ioPadsSpec = [ ]
            ioPinsSpec = [ (IoPin.WEST |IoPin.A_BEGIN, 'accumulator_out({})'  , 86*m1pitch, 86*m1pitch, (0,4) )
                         , (IoPin.EAST |IoPin.A_BEGIN, 'accumulator_out({})'  , 86*m1pitch, 86*m1pitch,  (4,8))


                         , (IoPin.NORTH |IoPin.A_BEGIN, 'multiplier({})'   , 86*m2pitch, 86*m2pitch, 4)
                         , (IoPin.SOUTH |IoPin.A_BEGIN, 'multiplicand({})'   , 84*m2pitch, 84*m2pitch, 4)
                         
                         , (IoPin.SOUTH|IoPin.A_BEGIN, 'clk'     , (84+4)*m2pitch,       0 ,  1)
                         , (IoPin.SOUTH|IoPin.A_BEGIN, 'reset'   , (84+4+1)*m2pitch,       0 ,  1)
                         ]
